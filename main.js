...
function pollActions(cron) {
	Object.keys(actions).forEach(id => actions[id].cron === cron && addTask({ name: 'sendCommand', id }));
...
function startAdapter(options) {
	...
	adapter.on('objectChange', (id, obj) => {
		if (
			...
			if (!actions[id]) {
				adapter.log.info(
					`enabled polling of ${id} with schedule ${obj.common.custom[adapter.namespace].schedule}`,
				);
					...
					parseString(xmlStringSerialized, { explicitArray: true, mergeAttrs: true }, (err, result) => {
						...
						if (err) {
							...
							if (!result || !result.root || !result.root.device) {
								adapter.log.debug(
									`Error by parsing of ${strLocation}: Cannot find deviceType in ${JSON.stringify(result)}`,
								);
								return;
							}
							...
							let strPort = strLocation.replace(/\bhttp:\/\/.*\d:/gi, '');
							strPort = strPort.replace(/\/.*/g, '');
							if (strPort.match(/http:/gi)) {
								strPort = '';
							} else {
							...
							strLocation = strLocation.replace(/http:\/\/|"/g, '').replace(/:\d*\/.*/gi, '');
							...
							// Add to Alives list
							getIDs().then(
								result => result.alives.indexOf(aliveID) === -1 && result.alives.push(aliveID),
							);
							...
							if (path.deviceList && path.deviceList[0].device) {
								...
								if (lenSubDevices) {
									...
									for (i = lenSubDevices - 1; i >= 0; i--) {
										// Looking for deviceType of device
										try {
											xmlDeviceType = getValueFromArray(
												path.deviceList[0].device[i].deviceType,
											).replace(/"/g, '');
											xmlTypeOfDevice = xmlDeviceType.replace(/:\d/, '');
											xmlTypeOfDevice = xmlTypeOfDevice.replace(/.*:/, '');
											xmlTypeOfDevice = nameFilter(xmlTypeOfDevice);
										...
										try {
											xmlfriendlyName = getValueFromArray(
												path.deviceList[0].device[i].friendlyName,
											);
										...
										//Looking for the manufacturer of a device
										try {
											xmlManufacturer = getValueFromArray(
												path.deviceList[0].device[i].manufacturer,
											).replace(/"/g, '');
											// eslint-disable-next-line @typescript-eslint/no-unused-vars
										} catch (err) {
										...
										//Looking for the manufacturerURL
										try {
											xmlManufacturerURL = getValueFromArray(
												path.deviceList[0].device[i].manufacturerURL,
											);
											// eslint-disable-next-line @typescript-eslint/no-unused-vars
										} catch (err) {
										...
										//Looking for the modelNumber
										try {
											xmlModelNumber = getValueFromArray(
												path.deviceList[0].device[i].modelNumber,
											);
											// eslint-disable-next-line @typescript-eslint/no-unused-vars
										} catch (err) {
										...
										//Looking for the modelDescription
										try {
											xmlModelDescription = getValueFromArray(
												path.deviceList[0].device[i].modelDescription,
											);
											// eslint-disable-next-line @typescript-eslint/no-unused-vars
										} catch (err) {
										...
										getIDs().then(
											result =>
												result.alives.indexOf(aliveID) === -1 && result.alives.push(aliveID),
										);
										...
										if (
											...
											//Counting SubDevices
											let i_SubSubDevices =
												path.deviceList[0].device[i].deviceList[0].device.length;
											let i2;
											...
											if (i_SubSubDevices) {
												for (i2 = i_SubSubDevices - 1; i2 >= 0; i--) {
													...
													try {
														...
														adapter.log.debug(
															`Can not read friendlyName of SubDevice from ${xmlFN}`,
														);
														xmlfriendlyName = 'Unknown';
													}
													//Looking for the manufacturer of a device
													try {
														...
														adapter.log.debug(
															`Can not read manufacturer of ${xmlfriendlyName}`,
														);
														xmlManufacturer = '';
													}
													//Looking for the manufacturerURL
													try {
														...
														adapter.log.debug(
															`Can not read manufacturerURL of ${xmlfriendlyName}`,
														);
														xmlManufacturerURL = '';
													}
													//Looking for the modelNumber
													try {
														...
														adapter.log.debug(
															`Can not read modelNumber of ${xmlfriendlyName}`,
														);
														xmlModelNumber = '';
													}
													//Looking for the modelDescription
													try {
														...
														adapter.log.debug(
															`Can not read modelDescription of ${xmlfriendlyName}`,
														);
														xmlModelDescription = '';
													}
													//Looking for deviceType of device
													try {
														...
														adapter.log.debug(
															`Can not read DeviceType of ${xmlfriendlyName}`,
														);
														xmlDeviceType = '';
													}
													//Looking for the modelName
													try {
														xmlModelName = getValueFromArray(
															path.deviceList[0].device[i].deviceList[0].device[i2]
																.modelName,
														);
														// eslint-disable-next-line @typescript-eslint/no-unused-vars
													...
													try {
														...
														adapter.log.debug(
															`Can not read modelURL of ${xmlfriendlyName}`,
														);
														xmlModelURL = '';
													}
													//Looking for UDN of a device
													try {
														xmlUDN = getValueFromArray(
															path.deviceList[0].device[i].deviceList[0].device[i2].UDN,
														)
															.replace(/"/g, '')
															.replace(/uuid:/g, '');
														// eslint-disable-next-line @typescript-eslint/no-unused-vars
													...
													getIDs().then(
														result =>
															result.alives.indexOf(aliveID) === -1 &&
															result.alives.push(aliveID),
													);
												}
											}
										}
...
function createServiceList(result, xmlFN, xmlTypeOfDevice, object, strLocation, strPort, path) {
	...
	for (i = i_services - 1; i >= 0; i--) {
		...
		// Add to SID list
		getIDs().then(result => result.sids.indexOf(sid) === -1 && result.sids.push(sid));
	...
	try {
		for (let i2 = iStateVarLength - 1; i2 >= 0; i2--) {
			...
			try {
				...
				strAllowedValues = Object.keys(allowed)
					.map(xmlAllowedValue => allowed[xmlAllowedValue])
					.join(' ');
				// eslint-disable-next-line @typescript-eslint/no-unused-vars
			} catch (err) {
...
function startServer() {
	...
	server.on('advertise-alive', headers => {
		let usn = getValueFromArray(headers['USN']).replace(/uuid:/gi, '').replace(/::.*/gi, '');
			...
			if (discoveredDevices.indexOf(location) === -1) {
				...
				adapter.getDevices((err, devices) => {
					...
					for (let i = 0; i < devices.length; i++) {
						if (!devices[i] || !devices[i].native || !devices[i].native.uuid) {
							continue;
						}
...
async function subscribeEvent(id, cb) {
	const service = id.replace(/\.Alive/gi, '');
...
function listener(eventUrl, channelID, infoSub, subKey) {
	...
	infoSub.on('subscribed', data => {
		variableTimeout += 5;
		setTimeout(
			() => adapter.setState(`${channelID}.sid`, { val: ((data && data.sid) || '').toString(), ack: true }),
			variableTimeout,
		);
	...
	infoSub.on('message', data => {
		...
		setTimeout(
			() => getIDs().then(result => lookupService(data, JSON.parse(JSON.stringify(result.sids)))),
			variableTimeout,
		);
	...
	if (!SIDs || !SIDs.length || !data || !data.sid) {
		...
		adapter.getState(id, (err, state) => {
			if (err || !state || typeof state !== 'object') {
				...
				setNewState(state, id.replace(/\.sid$/, ''), data, () => setImmediate(lookupService, data, SIDs, cb));
			}
		});
	}
}
...
function setNewState(state, serviceID, data, cb) {
	...
	if (valueSID !== null && valueSID !== undefined) {
		...
		if (valueSID.indexOf(data.sid.toString().toLowerCase()) !== -1) {
			...
			} else if (newStates2.match(/<Event.*/gi)) {
				parseString(newStates, (err, result) => {
					...
					if (states) {
						for (let x = states.length - 1; x >= 0; x--) {
							...
							val = strState.match(/val":"(\w*(:\w*|,\s\w*)*)/gi);
							if (val) {
								val = val[0];
								val = val.replace(/val":"/gi, '');
							}
...
function valChannel(strState, serviceID, cb) {
	//looking for the value of channel
	let channel = strState.match(/channel":"(\w*(:\w*|,\s\w*)*)/gi);
	if (channel) {
		channel = channel.toString();
		channel = channel.replace(/channel":"/gi, '');
...
function valLookup(strState) {
	let val = strState.match(/\w*":"(\w*\D\w|\w*|,\s\w*|(\w*:)*(\*)*(\/)*(,)*(-)*(\.)*)*/gi);
	if (val) {
		val = val.toString();
		val = val.replace(/"\w*":"/i, '');
		val = val.replace(/"/gi, '');
...
function convertEventObject(result) {
	const regex = new RegExp(/"\w*":\[{"\$":{("\w*":"(\w*:\w*:\w*|(\w*,\s\w*)*|\w*)"(,"\w*":"\w*")*)}/gi);
...
function convertWM(result) {
	const regex = new RegExp(
		/"\w*":{".":"(\w*"|((http-get:\*:\w*\/((\w*\.)*|(\w*-)*|(\w*\.)*(\w*-)*)\w*:\w*\.\w*=\w*(,)*)*((http-get|rtsp-rtp-udp):\*:\w*\/(\w*(\.|-))*\w*:\*(,)*)*))/g,
	);
	return (JSON.stringify(result) || '').match(regex);
}
...
function readSchedules() {
	return new Promise(resolve => {
		adapter.getObjectView(
			'system',
			'custom',
			{ startkey: `${adapter.namespace}.`, endkey: `${adapter.namespace}.\uFFFF` },
			(err, doc) => {
				if (doc && doc.rows) {
					for (let i = 0, l = doc.rows.length; i < l; i++) {
						if (doc.rows[i].value) {
							...
							if (
								...
								obj[adapter.namespace] &&
								obj[adapter.namespace].enabled &&
								id.match(/\.request$/)
							) {
...
function createMessage(sType, aName, _ip, _port, cURL, body, actionID, cb) {
	...
	fetch(url, options)
		.then(async res => {
			...
			if (!res.ok) {
				adapter.log.warn(
					`Unexpected answer from upnp service: ${res.status}\n Sent message: ${JSON.stringify(options)}`,
				);
				cb && cb();
			} else {
				...
				parseString(body, null, (err, result) => {
					...
					try {
						...
						if (response) {
							const actionResponseName = Object.keys(response).find(key =>
								key.includes(`${aName}Response`),
							);
